...
const BLOG_NAME = process.env.BLOG_NAME || '云梯笔记';
...
function header() {
  return `<header class="site-header"><div class="site-header-inner">
    <a class="site-brand" href="/"><span class="site-brand-icon" aria-hidden="true">🌐</span><span class="site-brand-copy"><span class="site-brand-name">${escapeHtml(BLOG_NAME)}</span><span class="site-brand-note">CYBER CLOUD NODE</span></span></a>
...
function footer() {
  return `<footer class="site-footer"><div class="site-footer-inner"><div><div class="footer-brand">${escapeHtml(BLOG_NAME)}</div><div class="footer-saying"><span class="cyber-tag">GLOBAL TUNNEL</span> 穿透信息迷雾 · 赋能数字自由</div></div><div class="footer-meta">© ${new Date().getFullYear()} · Notion Synced · <a href="/rss.xml">[RSS FEED]</a></div></div></footer>`;
}

export function layout({ title = BLOG_NAME, description = SITE_DESCRIPTION, body, bodyClass = '' }) {
  return `<!DOCTYPE html><html lang="zh-CN" data-theme="dark"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=5.0"><title>${escapeHtml(title)} · ${escapeHtml(BLOG_NAME)}</title><meta name="description" content="${escapeHtml(description)}"><meta property="og:title" content="${escapeHtml(title)}"><meta property="og:description" content="${escapeHtml(description)}"><meta property="og:type" content="article"><link rel="icon" href="data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 viewBox=%220 0 100 100%22><text y=%22.9em%22 font-size=%2290%22>🌐</text></svg>"><link rel="stylesheet" href="/styles.css"><link rel="stylesheet" href="/liquid.css?v=7"><link rel="alternate" type="application/rss+xml" title="${escapeHtml(BLOG_NAME)}" href="/rss.xml">${themeScript()}</head><body class="${escapeHtml(bodyClass)}"><div class="glass-bg" aria-hidden="true"><span class="glass-blob blob-1"></span><span class="glass-blob blob-2"></span><span class="glass-blob blob-3"></span></div><div class="reading-progress"><div class="bar"></div></div>${header()}<main class="site-main">${body}</main>${footer()}<script src="/main.js"></script></body></html>`;
}
...
export function homePage(posts, stats) {
  ...
  return layout({ title: BLOG_NAME, description: SITE_DESCRIPTION, body, bodyClass: 'is-home' });
}
